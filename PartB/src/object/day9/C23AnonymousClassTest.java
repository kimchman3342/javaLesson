...
/* 인터페이스 중에 **추상메소드가 1개인 것**은 다른 프로그래밍 언어에서 사용되는 '함수'의 형식으로 표현할 수 있습니다.
    함수와 메소드의 공통점은 인자와 리턴 값이 있다는 것. 다른 점은 메소드는 클래스(또는 객체)에 구성요소
    *자바에서 함수형 인터페이스를 익명클래스 형식으로 많이 사용합니다. 람다식 사용가능*
    

 *자바 라이브러리에는 인자의 형식과 개수와 리턴 타입 등 여러 유형이 따라
 * Consumer, Producer(자바 8까지 이후 Supplier), Function, Predicate 등의 함수형 인터페이스가 미리 정의 되어 있습니다.
 * 
