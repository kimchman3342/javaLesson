...
-- ※ step 2 ※ : ※동등 조인※ 은 위의 결과 중 고객 ID 값이 같은 것만 의미가 있으므로 조건식을 추가한 것 입니다.
SELECT * 
FROM TBL_CUSTOM tc ,TBL_BUY tb 
WHERE tc.CUSTOM_ID = tb.CUSTOMID ;
-- 참고 : 고객 중 구매를 한 번도 하지 않았다면 조인 결과에 없습니다.
...
-- 2) 테이블을 바꾼 예시 : 회원 'twice'가 '구매'한 수량, '상품명과 가격'을 조회하세요.
SELECT tp.PNAME ,tb.QUANTITY, tp.PRICE , tb.QUANTITY * tp.PRICE "구매금액" 
FROM TBL_PRODUCT tp, TBL_BUY tb  
WHERE tp.PCODE  = tb.PCODE
AND tb.CUSTOMID = 'twice';

-- 3) 2개의 테이블을 조인하는 예시 생각해보세요.	B1 가테고리 상품의 구매 현황(상품명, 가격, 구매수량, 구매날짜)을 조회하세요.
SELECT tp.PNAME , tp.PRICE , tb.QUANTITY , tb.BUY_DATE 
FROM TBL_PRODUCT tp ,TBL_BUY tb 
WHERE tp.PCODE = tb.PCODE 
AND tp.CATEGORY = 'B1';

-- 동등 조인 형식 1) select * from 테이블 1 t1, 테이블 2 t2 where t1.공통컬럼 = t2.공통컬럼;
--			   2) select * from 테이블 1 t1 join 테이블 2 on t1.공통컬럼 = t2.공통컬럼;
SELECT *
FROM TBL_PRODUCT tp 
JOIN TBL_BUY tb 
...
-- 외부 조인 : 
 /*select 
 	...
 	on t1.공통컬럼 = t2.공통컬럼;
 	테이블2에 없는 값도 테이블 1에 있으면 조인에 포함시킵니다. 테이블2에 해당하는 컬럼 값은 null입니다.
 	**/
SELECT * 
FROM TBL_CUSTOM tc 
