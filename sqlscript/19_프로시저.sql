/* 	SQL은 2월 1일에 SELECT 10문제
 *  팀별 예시 문제 : 출제 1월 25일
 				: 이런 것은 '비즈니스 로직에 필요하다'
 	요구사항 : 고객은 custom 테이블에, 상품은 product 테이블에 저장된 값만 buy 테이블에 사용 가능.
 			구매는 '동일한 고객이 동안한 상품을 여러번 구매할 수 있다.' => PK를 idx 컬럼으로 추가함.
 * */
...
--이름에 '길동'이 들어가는 회원의 구매한 상품의 구매현황 조회?(권태윤)
...
-- 수정
SELECT PCODE, tb.QUANTITY, tb.BUY_DATE 
FROM TBL_CUSTOM tc 
JOIN TBL_BUY tb 
ON tc.CUSTOM_ID = tb.CUSTOMID 
WHERE name = '홍길동';

-- 1) '20살'이상 고객님들의 '구매'한 '상품명'(강주찬)  => 2)테이블 2개 조인으로 문제를 변경하세요.
SELECT tp.*
FROM TBL_PRODUCT tp 
JOIN TBL_BUY tb 
...
-- 수정
SELECT CUSTOMID ,tc.AGE, tb.PCODE 
FROM TBL_BUY tb 
JOIN TBL_CUSTOM tc  
ON tb.CUSTOMID = tc.CUSTOM_ID  
WHERE tc.AGE > 20;
...
-- 총 구매합산 금액이 20000~70000사이 값을 출력하시오(김태완)
SELECT SUM(price) 
(SELECT * 
FROM TBL_PRODUCT tp 
JOIN TBL_BUY tb 
...
GROUP BY total.PCODE, PNAME; -- 서브쿼리 없이 변경하세요.
...
-- 한진만 문제 : '팔린 갯수가 가장 많은 순서'로 (상품코드로 그룹화)로 상품(상품코드와 상품이름)을 정렬하고 총 팔린 금액을 출력하시오
...
-- 수정
SELECT tp.PRICE"총 팔린금액"
FROM 
	(SELECT tb.PCODE , tp.PNAME ,tb.QUANTITY ,tp.PRICE
	FROM TBL_PRODUCT tp
	LEFT JOIN TBL_BUY tb 
	ON tp.PCODE = tb.PCODE)
;
...
-- 황병훈 문제 : 진라면을 구매한 고객들의 평균 나이를 제품코드(PCODE)와 함께 출력해 주세요.
...
--조지수 문제 : '30이하' 회원별 구매 금액('단가','수량')을 구하고 회원으로 그룹바이해서 구매금액 합계가 큰 순으로 정렬
...
FROM (
SELECT tb.QUANTITY * tp.PRICE 
AS TOTAL, BUY_DATE , tb.CUSTOMID, QUANTITY,PRICE
FROM TBL_PRODUCT tp, TBL_BUY tb
WHERE tp.PCODE = tb.PCODE
