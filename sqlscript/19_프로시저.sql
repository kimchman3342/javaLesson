...
BEGIN		--프로시저 시작(실행부)
  -- 프로시저 내부에는 주로 트랜잭션을 구성하는 여러개의 DML 명령문들을 작성
	SELECT name,age 
		INTO vname , vage	-- 프로시저 구문: 검색결과를 프로시저 일반 변수 vname, vage 에 저장
	FROM "TBL_CUSTOM" tc 
	WHERE CUSTOM_ID ='hong';		-- 1개 행만 결과 조회되는 조건
...
/*
	...
	( 매개변수이름 [IN 또는 OUT] 데이터 타입,.....)	IN : 입력매개변수, OUT : 출력매개변수(리턴 값 저
	[IS 또는 AS]
		프로시저 변수 선언
	BEGIN
		프로시저 작업 내용 SQL들....
...
BEGIN 
   ...
   WHEN no_data_found THEN   
      DBMS_OUTPUT.PUT_LINE(CHR(10_'찾는 데이터가 없습니다.');   
END;   
...
-- EXEC SEARCH_custom('hong');	-- 디비버에서는 실행 안됩니다.
...
-- 복습 : 프로시저는 SQL로 만든 프로그램. => 여러 개의 DML로 구성이 됩니다. PLSQL이라고 부릅니다.
...
BEGIN
   SELECT max(quantity)		-- 구매 수량 최대 값
      INTO maxval 			-- 조회 결과를 변수에 저장
   FROM tbl_buy; 
...
DECLARE
	...
	vage tbl_custom.age %TYPE;
BEGIN
	max_custom(vname,vage);		-- 프로시저 실행 결과 값을 vname, vage 변수에 저장합니다.
	DBMS_OUTPUT.PUT_LINE(CHR(10));	-- 줄 바꿈
	DBMS_OUTPUT.PUT_LINE('=고객이름 : ' || vname);	-- 줄 바꿈
	DBMS_OUTPUT.PUT_LINE('=고객나이 : ' || vage);	-- 줄 바꿈
END;
	-- 줄 바꿈
