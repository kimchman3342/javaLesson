...
-- 필요시 참조관계일때 테이블 삭제 : 참조 관계의 부모테이블 '학생'은 drop 못합니다.
-- 2개의 테이블을 모두 삭제하려면 자식 테이블 drop 하고 부모님 테이블 drop순서로 합니다.
DROP TABLE TBL_STUDENT ;
DROP TABLE TBL_SCORE ;


-- 필요시 참조관계일때 테이블 데이터 삭제	: 참조되는 데이터가 성적테이블에 있으면 학생 테이블 데이터를 삭제 못합니다.
...
-- PK(행을 `식별`하는 역할) 컬럼을 조건으로 select 조회하면 조회되는 행의 개수는 항상 0(없을 때) 또는 1(있을 때)입니다.
...
SELECT * FROM TBL_SCORE ts WHERE subject = '과학';		-- 검색 결과 n개
...
- 성적테이블에 '과목명'을 학생 1명이 동일한 과목을 수강한다면(예를 들면 1학년 국어 ,  2학년 국어, 3학년 국어 )
  ...
  2) '과목' 테이블 - 과목코드(PK), 과목명(중복허용), 담당교사(중복허용)
