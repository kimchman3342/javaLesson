...
SELECT tb.CUSTOMID "구매 합산 100000~200000인 고객 ID"
FROM TBL_PRODUCT tp, TBL_BUY tb
WHERE tp.PCODE = tb.PCODE
AND tp.price*tb.QUANTITY BETWEEN 100000 AND 200000;


/*  B조 */
--B-1. 20대 나이 고객의 구매 상품 코드와 나이를 나이순으로 정렬 조회 (이대환) /* C조 조이루 
SELECT tb.PCODE "상품코드",tc.AGE "나이"
FROM TBL_CUSTOM tc ,TBL_BUY tb 
WHERE tc.CUSTOM_ID = tb.CUSTOMID
AND tc.AGE BETWEEN 20 AND 29
ORDER BY AGE ASC;

--B-2. 나이가 가장 많은 고객이 상품을 구매한 횟수를 조회하세요.-서브쿼리 사용하기 (김승한)

SELECT tc.NAME "이름",tc.AGE "나이" ,SUM(QUANTITY) "수량"
FROM TBL_BUY tb, TBL_CUSTOM tc 
WHERE tb.CUSTOMID =tc.CUSTOM_ID 
AND tc.AGE  = (SELECT MAX(AGE) FROM TBL_CUSTOM)
GROUP BY tc.NAME,tc.AGE;

 
--B-3. 2023년 하반기 구매금액을 고객ID별로 조회하시오. 금액이 높은 순서부터 조회하세요. (노희영)
SELECT tb.CUSTOMID "아이디", SUM(tb.QUANTITY*tp.PRICE) "총 구매금액"
  FROM TBL_PRODUCT tp ,TBL_BUY tb  
 WHERE tp.PCODE = tb.PCODE 
   AND tb.BUY_DATE BETWEEN TO_DATE('2023-06-01','yyyy-mm-dd')
					   AND TO_DATE('2023-12-31','yyyy-mm-dd')
 GROUP BY tb.CUSTOMID;

--B-4. 2024년에 구매횟수가 1회 이상인 고객id, 고객이름, 나이,이메일을 조회하세요.(이재훈)
SELECT custom_id "고객ID", name "이름", age "나이", email "이메일"
FROM TBL_CUSTOM tc 
WHERE CUSTOM_ID IN (
		SELECT tb.CUSTOMID
		FROM TBL_BUY tb 
...
SELECT tc.name "이름", tp.pname "상품",(tp.price*tb.quantity) "구매금액"
FROM TBL_CUSTOM tc , TBL_PRODUCT tp ,TBL_BUY tb 
WHERE tc.CUSTOM_ID =tb.CUSTOMID 
AND tb.PCODE = tp.PCODE
ORDER BY tb.CUSTOMID,tb.PCODE ASC;

/* C조 */
--C-1. 가격 1만원 이상의 상품에 대해 각각 고객들이 구매한 평균 개수를 출력하시오.상품코드 순서로 정렬 (임현범)
SELECT tp.PCODE "상품코드", tp.PNAME "상품명", avg(tb.QUANTITY) "평균 구매 개수"
	FROM TBL_BUY tb 
	JOIN TBL_PRODUCT tp 
	ON tb.PCODE = tp.PCODE
...
SELECT tc.NAME "이름", tb.QUANTITY "수량", tb.BUY_DATE "구매날짜"
FROM TBL_CUSTOM tc ,TBL_BUY tb 
WHERE tc.CUSTOM_ID = tb.CUSTOMID
...
SELECT tp.PNAME "상품명",tp.PCODE "상품코드", tb.QUANTITY "팔린 수량",tp.PRICE *tb.QUANTITY "판매금액"
FROM TBL_PRODUCT tp ,TBL_BUY tb 
WHERE tp.PCODE = tb.PCODE 
AND tb.BUY_DATE LIKE '23%';
...
SELECT tp.PNAME"상품", twiceGD.quantity "수량", tp.PRICE "가격"
FROM twiceGD
JOIN TBL_PRODUCT tp
ON twiceGD.PCODE = tp.PCODE 
ORDER BY price desc
;
...
SELECT tb.customid, tb.quantity * tp.price  AS sum
  ...
  ORDER BY SUM DESC;  
		...
		SELECT tb.CUSTOMID "아이디", SUM(tb.QUANTITY*tp.PRICE) "진라면 총 구매금액"
		  FROM TBL_PRODUCT tp ,TBL_BUY tb  
		 WHERE tp.PCODE = tb.PCODE 
		   AND tb.BUY_DATE BETWEEN TO_DATE('2023-06-01','yyyy-mm-dd')
							   AND TO_DATE('2023-12-31','yyyy-mm-dd')
...
SELECT tp.PNAME "상품명", tp.PCODE "상품코드", SUM(tp.PRICE*tb.QUANTITY) "판매금 합계"
FROM TBL_PRODUCT tp ,TBL_BUY tb 
WHERE tp.PCODE = tb.PCODE
GROUP BY tp.PCODE,tp.PNAME
ORDER BY SUM(tb.QUANTITY) DESC,tp.PCODE;

--D-3. 진라면을 구매한 고객들의 평균 나이를 제품코드(PCODE)와 함께출력해 주세요.(황병훈)
SELECT AVG(tc.age) "평균나이", tb.PCODE "상품코드"
FROM TBL_CUSTOM tc , TBL_BUY tb 
WHERE tc.CUSTOM_ID = tb.CUSTOMID 
AND tb.PCODE = 'JINRMn5'
GROUP BY tb.PCODE ;
